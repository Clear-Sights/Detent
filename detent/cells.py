...
or the reason it's void. The `detent.moves.*` rows are NOT hand-declared here — every move
...
# Hand-declared base: status, reason, and non-move machinery ONLY. Adding a detent.moves.* entry
...
_BASE: dict[int, dict] = {
    ...
    4:  {"flow": "USER→WORLD",        "status": "VOID",
         "reason": "Detent never initiates; the owner's own outward acts are not its writ"},
    ...
    11: {"flow": "WORKSPACE→STORE",   "status": "SERVED",
         "machinery": ("detent.store.put", "detent.store.put_file")},
    ...
    13: {"flow": "STORE→USER",        "status": "SERVED",
         "machinery": ("detent.store.materialize", "harness:send-file")},
    14: {"flow": "STORE→CONTEXT",     "status": "SERVED"},
    15: {"flow": "STORE→WORKSPACE",   "status": "SERVED",
         "machinery": ("detent.store.materialize",)},
    16: {"flow": "STORE→WORLD",       "status": "SERVED", "machinery": ("detent.store.get",)},
...
def coverage_failures() -> list[str]:
    ...
    for n, cell in sorted(CELLS.items()):
        for entry in cell.get("machinery", ()):
            if not entry.startswith("detent."):
