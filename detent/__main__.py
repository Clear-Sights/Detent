"""`python -m detent` — the one human-facing trace: is the rod latched, is the punchcard honest.

Prints, all machine-derived (nothing paraphrased): wiring (which active configs reference
detent's dispatch), the 20-cell coverage table straight from cells.CELLS, store stats from disk,
and every coverage failure. Exit 0 only when the punchcard matches reality — a status that
cannot return FALSE is theater, so this IS the check (`/detent` relays it verbatim).
"""
