...
| 19 | CONTEXT→WORKSPACE | Write/Edit: anchor resolved by declared cardinality (ambiguity = hard error, pre-checked before the call burns a round trip); no re-emission of bytes already on disk (small delta → Edit, not whole-file Write) | SERVED: `context_to_workspace` — pointers expand (Write at PreToolUse, proven live; Edit via the defer→PermissionRequest seam, the documented route around clients that validate old_string before pre-rewrites; probes 2026-07-10, sharpened: dispatch's defer envelope is emitted correctly (verified on the wire), but a raw-arrival tee on the PermissionRequest hook recorded ZERO deliveries for a pointer Edit in this session — the event is not delivered here at all, most plausibly because the session auto-approves edits (no permission evaluation → no request → defer collapses to the default flow and client validation fails the raw pointer, cleanly). Same probe shows FileChanged undelivered (no watchPaths at SessionStart). NOT a snapshot-at-start effect: Detent's own hooks were installed mid-session and fired immediately. Proof criteria: a session where Edit permission is evaluated (ask mode), and a session started with watchPaths; the arrival tees stay wired and will record the first delivery of either event), anchors face the cardinality gate, byte-identical re-emissions are denied |
