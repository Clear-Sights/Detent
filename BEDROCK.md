...
memoization. Topology intact; the sanctioned path's RESULT leg is now enforced (`subagent_result_capture`
...
| 8 | WORLD→STORE | fetches hash-cached; repeat fetch of same URL+hash is a STORE hit | SERVED: `response_capture_and_bound` captures every retrieval by hash — any tool, first contact |
...
| 17 | CONTEXT→USER | the reply: every fact machine-included from a trace/artifact; the model contributes reasoning only — never transported facts | PARTIAL at boundary ceiling — the checkable slice is fully enforced (`stop_capture_and_cite_check`: cited addresses resolve or the turn blocks; `display_materialize_citations` renders them); distinguishing transported facts from reasoning inside prose is judgment — the sibling faculty's writ. Quote-transport gate measured 0.0% and declined by the benefit rule |
| 18 | CONTEXT→WORLD | outward composed acts (PR bodies, comments, API calls) — verify-gated | PARTIAL at boundary ceiling — `outbound_deny_secret_pattern` gates the ENTIRE →WORLD tool class (every `mcp__*` tool by the pinned MCP-is-WORLD convention, plus WebFetch/WebSearch) on exact secret grammars; deciding which outbound claims need receipts is judgment — the sibling faculty's writ |
| 19 | CONTEXT→WORKSPACE | Write/Edit: anchor resolved by declared cardinality (ambiguity = hard error, pre-checked before the call burns a round trip); no re-emission of bytes already on disk (small delta → Edit, not whole-file Write) | SERVED: `edit_deny_ambiguous_anchor` + `write_deny_reemission` |
| 20 | CONTEXT→STORE | generation captured as artifact at emission; stochastic once, addressable forever | SERVED: `edit_write_capture` (every Edit/Write emission) + `stop_capture_and_cite_check` (every reply) |
